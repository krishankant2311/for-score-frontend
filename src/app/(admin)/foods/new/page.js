      ...
      <div className="mt-6 space-y-5 rounded-2xl border border-[#C8D7E9] bg-white p-6 shadow-md">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            ["Protein (g)", protein, setProtein, "g"],
            ["Carbs (g)", carbs, setCarbs, "g"],
            ["Fat (g)", fats, setFats, "g"],
          ].map(([label, val, setVal]) => (
            <div key={label}>
              <label className="text-sm font-medium text-[#0A3161]">{label}</label>
              <Input className="mt-1.5 h-12" value={val} onChange={(e) => setVal(e.target.value.replace(/[^\d.]/g, ""))} inputMode="decimal" />
            </div>
          ))}
        </div>
        <div>
