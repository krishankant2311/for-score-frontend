...
export default function StatusDoughnut({
  ...
  centerValue,
  centerLabel = "total",
  subtitle = "Distribution for selected timeframe",
}) {
  ...
  const centerDisplay =
    centerValue !== undefined && centerValue !== null ? Number(centerValue) : total;
  ...
  const centerTextPlugin = {
    ...
    beforeDraw(chart) {
      ...
      ctx.fillText(String(centerDisplay.toLocaleString()), x, y - 6);
      ctx.fillStyle = "hsl(var(--muted-foreground))";
      ctx.font = "500 12px Inter, ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto";
      ctx.fillText(centerLabel, x, y + 14);
      ctx.restore();
    },
  };
